...
export default function DashboardScreen({ transactions, user }) {
  ...
  return (
    <div className="screen" style={{ padding: '0 0 80px' }}>
      <div style={{ padding: '20px 20px 0', background: 'linear-gradient(180deg, var(--surface) 0%, transparent 100%)' }}>
        ...
        <div style={{ display: 'flex', gap: 4, background: 'var(--card)', borderRadius: 10, padding: 3, marginBottom: 16 }}>
          {['quincenal', 'mensual'].map(v => (
            <button key={v} onClick={() => setVista(v)} style={{ flex: 1, padding: '8px 0', borderRadius: 8, border: 'none', cursor: 'pointer', background: vista === v ? 'var(--card2)' : 'transparent', color: vista === v ? 'var(--gold)' : 'var(--text3)', fontSize: 13, fontWeight: 500, transition: 'all 0.2s ease' }}>
        ...
        {vista === 'quincenal' && (
          <div style={{ display: 'flex', gap: 8, marginBottom: 16 }}>
            {[1, 2].map(q => (
              <button key={q} onClick={() => setQuincena(q)} style={{ flex: 1, padding: '10px 0', borderRadius: 10, border: 'none', cursor: 'pointer', background: quincena === q ? 'var(--gold-bg)' : 'var(--card)', borderWidth: quincena === q ? 1 : 0.5, borderStyle: 'solid', borderColor: quincena === q ? 'var(--gold-dim)' : 'var(--border2)', transition: 'all 0.2s ease' }}>
                ...
                <div style={{ fontSize: 9, color: 'var(--text3)' }}>{q === 1 ? 'nóm. 28 ant.' : 'nóm. 14 cte.'}</div>
              </button>
            ))}
          </div>
        )}
        ...
        <div className="card-glass" style={{ padding: '18px 20px', marginBottom: 6 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
            ...
            <div style={{ textAlign: 'right' }}>
              ...
              <div style={{ width: 8, height: 8, borderRadius: '50%', background: pctTotal >= 1 ? 'var(--red)' : pctTotal >= 0.8 ? 'var(--amber)' : 'var(--teal)', marginLeft: 'auto', marginTop: 4, boxShadow: '0 0 8px ' + (pctTotal >= 1 ? 'var(--red)' : pctTotal >= 0.8 ? 'var(--amber)' : 'var(--teal)') }} />
            </div>
          </div>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 8 }}>
            <span style={{ fontSize: 10, color: 'var(--text3)' }}>Disponible ppto: {mask(mxn(Math.max(totalPresupuesto - totalGastado, 0)))}</span>
            <span style={{ fontSize: 10, color: 'var(--text3)' }}>Ppto: {mask(mxn(INGRESO_QUINCENAL))}</span>
          </div>
        </div>

        {/* Saldo real con ingresos */}
        {totalIngresos > 0 && (
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8, marginBottom: 6 }}>
            <div style={{ background: 'var(--green-bg)', borderRadius: 10, padding: '10px 12px', textAlign: 'center' }}>
              <p style={{ fontSize: 9, color: 'var(--teal)', marginBottom: 2 }}>INGRESOS</p>
              <p style={{ fontSize: 13, fontWeight: 600, color: 'var(--teal)', fontVariantNumeric: 'tabular-nums' }}>{mask(mxn(totalIngresos))}</p>
            </div>
            <div style={{ background: 'var(--red-bg)', borderRadius: 10, padding: '10px 12px', textAlign: 'center' }}>
              <p style={{ fontSize: 9, color: 'var(--red)', marginBottom: 2 }}>GASTOS</p>
              <p style={{ fontSize: 13, fontWeight: 600, color: 'var(--red)', fontVariantNumeric: 'tabular-nums' }}>{mask(mxn(totalGastado))}</p>
            </div>
            <div style={{ background: saldoReal >= 0 ? 'var(--green-bg)' : 'var(--red-bg)', borderRadius: 10, padding: '10px 12px', textAlign: 'center' }}>
              <p style={{ fontSize: 9, color: saldoReal >= 0 ? 'var(--teal)' : 'var(--red)', marginBottom: 2 }}>SALDO</p>
              <p style={{ fontSize: 13, fontWeight: 600, color: saldoReal >= 0 ? 'var(--teal)' : 'var(--red)', fontVariantNumeric: 'tabular-nums' }}>{mask(mxn(saldoReal))}</p>
            </div>
          </div>
        )}
      </div>
      ...
      <div style={{ padding: '12px 16px', display: 'flex', flexDirection: 'column', gap: 8 }}>
        ...
        {CATEGORIES.map(cat => {
          ...
          cat.subcategories.forEach(sub => {
            ...
            presupuesto += vista === 'quincenal' ? (quincena === 1 ? sub.q1 : sub.q2) : (sub.q1 + sub.q2)
          })
      ...
      <div style={{ margin: '8px 16px 0', padding: '12px 14px', background: 'var(--green-bg)', borderRadius: 10 }}>
        <p style={{ fontSize: 11, color: 'var(--teal)', fontWeight: 500, marginBottom: 4 }}>Logros de marzo 2026</p>
        <p style={{ fontSize: 11, color: 'var(--text2)' }}>TC BBVA *2251 liquidada ✓ · Renta abr-may-jun prepagada ✓</p>
      </div>
    </div>
  )
}
