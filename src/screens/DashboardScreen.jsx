...
const getQuincenaGastado = (transactions, catId, subId, quincena, mes) => {
  return transactions.filter(t => {
    ...
    return d.getMonth() === mes && tQ === quincena && t.categoriaId === catId && (!subId || t.subcategoriaId === subId)
        ...
        {totalIngresos > 0 && (
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8, marginBottom: 8 }}>
            <div style={{ background: 'var(--green-bg)', borderRadius: 10, padding: '10px 12px', textAlign: 'center' }}>
              ...
              <p style={{ fontSize: 13, fontWeight: 600, color: 'var(--teal)' }}>{mask(mxn(totalIngresos))}</p>
            </div>
            <div style={{ background: 'var(--red-bg)', borderRadius: 10, padding: '10px 12px', textAlign: 'center' }}>
              ...
              <p style={{ fontSize: 13, fontWeight: 600, color: 'var(--red)' }}>{mask(mxn(totalGastado))}</p>
            </div>
            <div style={{ background: saldoReal >= 0 ? 'var(--green-bg)' : 'var(--red-bg)', borderRadius: 10, padding: '10px 12px', textAlign: 'center' }}>
              ...
              <p style={{ fontSize: 13, fontWeight: 600, color: saldoReal >= 0 ? 'var(--teal)' : 'var(--red)' }}>{mask(mxn(saldoReal))}</p>
            </div>
          </div>
        )}
      </div>
